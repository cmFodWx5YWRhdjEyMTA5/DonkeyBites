...
SMTP_REPLY_TO = 'hooks.smtp-reply'
...
##This function return all commit in branch that not in source branch
def get_commit_list_in_branch_not_in(branch,source):
    p = subprocess.Popen(['git','log', branch , '--not' , source ], stdout=subprocess.PIPE)	
    return p.stdout.read()[:-1]
...
def get_push_user():
    return os.environ['REMOTE_USER'] or ""
